...
void imprimirAssembler(FILE *pFinal,t_arbol root);
int recorrerArbol(FILE *pFinal,t_arbol root);
...
{ 
    ...
    izq = recorrerArbol(pFinal,root->izq); // LLAMO AL HIJO IZQUIERDO
...
void imprimirAssembler(FILE*pFinal,t_arbol root) {
    ...
    if(esComparador(root->dato)){
        ...
        if(anteriorAuxFlag == 1) {
            ...
            //insertArray(&array_nombres_variables,@aux1);
